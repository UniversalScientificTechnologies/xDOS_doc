...
With the recent firmware, the detector can be powered off with the 'MCU button'. Press and hold it. The button is sampled only once per measurement cycle, so it may take up to about ten seconds (one integration interval) before the device responds with a tone and starts the shutdown countdown; LED1, LED2, and LED3 then switch off one by one over approximately three seconds. Keep the button held until all three LEDs are off - in the worst case, this can take up to about thirteen seconds - and then immediately **release** the button to power the device off; it then stays off. Releasing the button before the countdown finishes cancels the shutdown. If the button stays pressed (for example, because it is stuck or blocked), the device does **not** power off and returns to normal operation after about fifteen seconds. The device can also be powered off by removing the BATDATUNIT from the AIRDOS or by using special commands on the USB-C port. Inspect the [airdoscontrol utility](https://github.com/UniversalScientificTechnologies/AIRDOS04/tree/AIRDOS04A/sw) description for more details.
...
For a description of all indicators, buttons, and LEDs on the front panel, see the [BATDATUNIT01 status indicator description](/airdos/BATDATUNIT01#status-indicator-description).
